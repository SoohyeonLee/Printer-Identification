...
def create_record(srcDir):
    ...
    for idx in range(len(file_list)):
        ...
        R_glcm = glcm(image=arr_img[:,:,0], distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4],levels=256).reshape([256,256,4])
        G_glcm = glcm(image=arr_img[:,:,1], distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4],levels=256).reshape([256,256,4])
        B_glcm = glcm(image=arr_img[:,:,2], distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4],levels=256).reshape([256,256,4])

        all_glcm = np.sum([R_glcm,G_glcm,B_glcm],axis=0)

        img_byte = all_glcm.tobytes()
